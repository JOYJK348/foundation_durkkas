...
import { LucidePlus, LucideSearch, LucideBookOpen, LucideMoreVertical, LucideX, LucideLoader2 } from 'lucide-react';
...
export default function CourseManagement() {
    ...
    const fetchCourses = async () => {
        try {
            ...
            // toast.error("Failed to load courses"); // Commented to avoid hydration mismatch toast
        } finally {
            setIsLoading(false);
        }
    };
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                <button
                    ...
                    className="btn-primary flex items-center gap-2 h-12 px-6 shadow-lg shadow-primary/20 hover:scale-105 transition-transform"
                >
                    <LucidePlus size={20} />
                    Create New Course
                </button>
                ...
                {!isLoading && courses.map((course) => (
                    <div key={course.id} className="group bg-white rounded-3xl border border-slate-200 overflow-hidden hover:shadow-2xl hover:shadow-primary/5 transition-all hover:translate-y-[-4px]">
                        <div className="h-48 bg-slate-100 relative">
                            {/* Random Gradient based on ID for visual variety */}
                    ...
                    <div className="bg-white w-full max-w-lg rounded-3xl shadow-2xl p-6 md:p-8 animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleCreateCourse} className="space-y-4">
                            ...
                            <div className="pt-4">
                                ...
                                >
                                    {isSubmitting ? <LucideLoader2 className="animate-spin w-5 h-5 mx-auto" /> : "Create Course"}
                                </button>
                            </div>
                        </form>
                    </div>
