...
export default function CompanyAdminDashboard() {
    ...
    const loadDashboardData = async () => {
        try {
            ...
            const [companiesData, branchesData, employeesData, departmentsData, designationsData, usersData, usageData] = await Promise.all([
                ...
                platformService.getCompanyUsage()
            ]);

            const currentCompany = companiesData[0];
            setCompany(currentCompany);

            // Use unified usage data from API
            const currentUsage: Usage = {
                users: usageData?.active_users || 0,
                employees: usageData?.active_employees || 0,
                branches: usageData?.active_branches || 0,
                departments: usageData?.active_departments || 0,
                designations: usageData?.active_designations || 0
            };
            ...
            const planName = currentCompany?.subscription_plan || 'TRIAL';
            if (planName === 'TRIAL' && currentCompany?.subscription_start_date) {
                const startDate = new Date(currentCompany.subscription_start_date);
            ...
            // Summary Cards with Limits from Company record
            const newStats: KPIStat[] = [
                {
                    label: "Branches",
                    value: currentUsage.branches,
                    subtext: maxBranches === 0 ? "Unlimited" : `of ${maxBranches} allowed`,
                    icon: MapPin,
                    color: "text-blue-600",
                    bgColor: "bg-blue-50",
                    limit: maxBranches || -1, // Use -1 to denote unlimited for progress calculation
                    current: currentUsage.branches
                },
                {
                    label: "Users",
                    value: currentUsage.users,
                    subtext: maxUsers === 0 ? "Unlimited" : `of ${maxUsers} allowed`,
                    icon: Users,
                ...
                {
                    label: "Departments",
                    value: currentUsage.departments,
                    subtext: "Business units",
                    icon: Layers,
    ...
    const planInfo = PLAN_INFO[planName] || PLAN_INFO['TRIAL'];
        ...
        <DashboardLayout>
            <div className="space-y-6 md:space-y-8 pb-12 px-4 md:px-0">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-sm md:text-base text-slate-600 font-medium">
                            Managing {usage.branches} location{usage.branches !== 1 ? 's' : ''} with {usage.employees} team members
                        </p>
                    </div>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
                                ...
                                <div className="flex items-end gap-1">
                                    ...
                                    <span className="text-sm text-white/60 mb-1">
                                        / {(company?.max_users === 0 || company?.max_employees === 0) ? '∞' : (company?.max_users || 10)}
                                    </span>
                                </div>
                                {company?.max_users && company.max_users > 0 && (
                                    <div className="mt-2 h-1.5 bg-white/20 rounded-full overflow-hidden">
                                        <div
                                            className={`h-full rounded-full transition-all ${getUsagePercentage(usage.users, company.max_users) >= 100 ? 'bg-rose-400' : getUsagePercentage(usage.users, company.max_users) >= 80 ? 'bg-amber-400' : 'bg-emerald-400'}`}
                                            style={{ width: `${Math.min(100, getUsagePercentage(usage.users, company.max_users))}%` }}
                                        />
                                    </div>
                                )}
                            </div>
                            <div className="bg-white/10 backdrop-blur-sm rounded-2xl p-4 border border-white/10">
                                ...
                                <div className="flex items-end gap-1">
                                    ...
                                    <span className="text-sm text-white/60 mb-1">
                                        / {company?.max_branches === 0 ? '∞' : (company?.max_branches || 1)}
                                    </span>
                                </div>
                                {company?.max_branches && company.max_branches > 0 && (
                                    <div className="mt-2 h-1.5 bg-white/20 rounded-full overflow-hidden">
                                        <div
                                            className={`h-full rounded-full transition-all ${getUsagePercentage(usage.branches, company.max_branches) >= 100 ? 'bg-rose-400' : getUsagePercentage(usage.branches, company.max_branches) >= 80 ? 'bg-amber-400' : 'bg-emerald-400'}`}
                                            style={{ width: `${Math.min(100, getUsagePercentage(usage.branches, company.max_branches))}%` }}
                                        />
                                    </div>
                                )}
                            </div>
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-3">
                                {ALL_MODULES.map((mod) => {
                                    ...
                                    const isEnabled = myModules.includes(mod);
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                    {stats.map((stat, idx) => {
                        const percentage = stat.limit ? getUsagePercentage(stat.current || 0, stat.limit) : 0;
                        const limitReached = stat.limit ? isLimitReached(stat.current || 0, stat.limit) : false;
