/**
 * CORE API - City Operations (Individual)
 * Route: /api/core/cities/[id]
 */
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        const { data: oldData } = await supabase.from('cities').select('*').eq('id', id).single();
        if (!oldData) return errorResponse(null, 'City not found', 404);
        ...
        return successResponse(data, 'City updated');
    } catch (error: any) {
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        return successResponse(null, 'City archived');
    } catch (error: any) {
        return errorResponse(null, error.message);
    }
}
